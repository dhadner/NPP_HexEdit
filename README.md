...
- **A macOS port** (`macos/`) — a fresh rewrite that runs as a plugin file
  (`HexEditor.dylib`) inside [Nextpad++](https://github.com/nextpad-plus-plus/nextpad-plus-plus-macos).
...
The build expects to find a checkout of `nextpad-plus-plus` next to this
repo (so they share a parent folder). If yours lives somewhere else, pass
`-DNPP_MACOS_DIR=/path/to/nextpad-plus-plus` on the first `cmake -S …`
...
[![Tests](docs/test-status/badge.svg)](docs/test-status.md)

Click the badge for the full per-tier dashboard. The badge is regenerated by `macos/scripts/pre-commit-tests.sh` and committed alongside [docs/test-status.md](docs/test-status.md). The relative path is deliberate — GitHub resolves it against whichever branch you're viewing, so the badge on `master` shows master's last verified state and the badge on `macos` shows macos's, independently.
