...
- **15 .strings files** covering 11 full translations and 4 regional overrides:
  - **Full:** `en` (canonical), `de` (German), `es` (Spanish), `fr` (French), `it` (Italian), `pl` (Polish), `ru` (Russian), `uk` (Ukrainian), `zh-Hans` (Simplified Chinese), `he` (Hebrew, RTL), `ar` (Arabic, RTL).
...
- **[LOCALIZATION.md](LOCALIZATION.md)** is a translator-facing guide covering the .strings file format, placeholders, the cascade, RTL handling, and how to test a new language locally with `defaults write org.notepadplusplus.mac AppleLanguages -array <tag>`. The About dialog links directly to it on GitHub. The guide also documents reusable patterns for plugin and host developers (CFPreferencesCopyAppValue trick, language-override env var, LTR-pin for data views).
- **RTL support.** Menus, NSAlert dialogs, and the host's window chrome auto-flip under Hebrew / Arabic locales (free from AppKit). The hex view itself is deliberately pinned LTR — Offset on the leading-physical edge, ASCII on the trailing-physical edge — because hex dumps are universally read left-to-right even by RTL-language developers. The Options dialog's four tabs (Start Layout, Startup, Colors, Font) flip via a coordinate-mirroring helper (`hexFlippedX` / `hexFlippedAlignment`) so manual-frame layouts reflect about the container midline under RTL: labels move from leading- to trailing-physical, help buttons from trailing- to leading-physical. The plugin's RTL detection (`hexCurrentLayoutIsRTL`) consults the same language preference the .strings cascade uses, so the plugin flips correctly even when the host system is English but the plugin is configured for Hebrew via `defaults write org.notepadplusplus.mac AppleLanguages -array he`. Verified by `testHexTableStaysLTRUnderHebrewLocale` (table stays LTR) and `testOptionsDialogFlipsUnderHebrew` (column-count label.minX > field.minX, opposite of LTR).
